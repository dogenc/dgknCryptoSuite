...
namespace dgkn::gui {
    ...
    QWidget* MainWindow::buildDashboardTab() {
        ...
        // Bewusst KEINE "MOUNTS aktiv"-Kachel: Ein Zähler eingehängter Volumes würde
        // die Existenz eines gemounteten Hidden Volumes verraten und damit die
        // plausible deniability (Glaubhafte Abstreitbarkeit) untergraben. Die Zahl der
        // aktiven Mounts wird daher absichtlich NICHT in der GUI angezeigt.
        v->addLayout(tiles);
    ...
    void MainWindow::onMountContainer() {
        ...
        QApplication::setOverrideCursor(Qt::WaitCursor);
        log("Mounte Container... (Argon2id-Ableitung + WinFsp, bitte warten)");
        // WICHTIG: mount_volume() ist langsam (Argon2id 256 MiB) und kann beim WinFsp-
        // Einhaengen mehrere Sekunden pro Laufwerksbuchstabe warten. Frueher lief das
        // synchron im UI-Thread -> die App fror ein ("reagiert nicht", Windows-Hang).
        // Wir fuehren den Mount daher in einem Worker-Thread aus und halten das UI mit
        // einer lokalen Event-Loop responsiv, bis das Ergebnis vorliegt.
        core::OpResult r{false, ""};   // sicher initialisiert (kein uninitialisiertes ok)
        {
            std::string c_path = path.toStdString();
            std::string c_mode = mode.toStdString();
            QEventLoop loop;
            std::thread worker([&]() {
                r = controller_.manager().mount_volume(c_path, s_pw, "", c_mode, secret);
                // Geheimnisse im Worker wipen, sobald sie nicht mehr gebraucht werden.
                crypto_utils::secure_wipe_string(s_pw);
                crypto_utils::secure_wipe_string(secret);
                // Event-Loop im UI-Thread beenden (threadsicher via queued call).
                QMetaObject::invokeMethod(&loop, "quit", Qt::QueuedConnection);
            });
            loop.exec();      // UI bleibt responsiv; kehrt zurueck, wenn der Worker fertig ist
            worker.join();
        }
        QApplication::restoreOverrideCursor();
    ...
    void MainWindow::onUnmountContainer() {
        ...
        // Bewusst NEUTRALE Eintraege: weder Containername noch der "normal"/"hidden"-Modus
        // werden angezeigt. Ein Label wie "HIDDEN - vault.dgkn" wuerde die Existenz eines
        // gemounteten Hidden Volumes verraten und damit die plausible deniability
        // (Glaubhafte Abstreitbarkeit) untergraben — genau wie es die entfernte
        // MOUNTS-Kachel getan haette. Angezeigt wird nur der Laufwerksbuchstabe; sind
        // mehrere Volumes offen, unterscheidet eine reine Durchnummerierung sie, ohne
        // irgendeine identifizierende Information preiszugeben.
        QStringList items; std::vector<std::string> ids;
        int n = 0;
        for (const auto& mv : mounts) {
            ++n;
            QString item = QString("Volume %1").arg(n);
            if (!mv.mount_point.empty())
                item += "  ·  " + QString::fromStdString(mv.mount_point);
            items << item;
