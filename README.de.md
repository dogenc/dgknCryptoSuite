...
Passwort **plus** dem gebundenen 2FA-Geheimnis (und optional TPM-/Geräte-Bindung) öffnen
lassen. Für **Einzeldatei-Verschlüsselung** lässt sich zusätzlich eine **optionale
Schlüsseldatei** (beliebige Datei) als zweiter Schlüsselfaktor binden.
...
| 🔒 **Mehrfaktor-Bindung** | Passwort + 2FA-Geheimnis (+ optional TPM/Geräte-ID); Einzeldateien zusätzlich mit optionaler Schlüsseldatei |
...
- **Virtuelles Laufwerk** (als Laufwerksbuchstabe einbinden) braucht zusätzlich den
  **WinFsp**-Treiber von <https://winfsp.dev> (keine Admin-Rechte zum Mounten nötig).
  Datei- und Container-Krypto funktionieren auch ohne.

## Lizenz

Der **eigene Code** von DGKN steht unter einer **Evaluations-Lizenz** (siehe
**[LICENSE](LICENSE)**): frei zum Herunterladen, Bauen, Ausführen und Testen für die
private, nicht-kommerzielle Evaluation. **Source-available, nicht Open Source** —
Weitergabe, Veränderung über das eigene Testen hinaus und kommerzielle Nutzung erfordern
vorherige schriftliche Erlaubnis. Die verwendeten Open-Source-Bibliotheken behalten ihre
eigenen Lizenzen — siehe **[THIRD-PARTY-NOTICES.txt](THIRD-PARTY-NOTICES.txt)**.
