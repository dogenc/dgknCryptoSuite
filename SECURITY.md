...
| Code signing | 🟡 release binaries are Authenticode-signed; cert is publisher-supplied | Sign `dgkn_gui.exe`, `dgkn_crypto.exe` and the installer with your code-signing certificate after building. EV cert recommended for immediate SmartScreen reputation. |
| CI / automated tests | ⚠️ local only | Put the 42-test suite in CI (GitHub Actions, Windows runner) so every commit is verified. |
...
| Aspect | Status |
|---|---|
| Data at rest | 🟢 Full-container AEAD; header looks random; no plaintext magic to fingerprint. **Measured Shannon entropy of a fresh container ≈ 7.91 bits/byte (8.0 = perfect random).** |
...
6. ✅ **Optional TPM 2.0 binding** — real TBS availability check (`Tbsi_GetDeviceInfo`, tpmVersion==2) + machine-bound sealing via DPAPI/LocalMachine with PCR-policy entropy. Sealed secrets don't transfer to another machine. Tested.
