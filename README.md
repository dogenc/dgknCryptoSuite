...
[![License](https://img.shields.io/badge/License-Evaluation-orange?style=flat-square)](LICENSE)
...
DGKN Crypto Suite is a local, security-focused **encrypted volume manager** for Windows, written entirely in **native C++** with a **Qt 6** GUI. Files are stored inside encrypted containers that can only be opened with the correct password plus the bound 2FA secret (and optional TPM / device binding). For **single-file encryption**, you can additionally bind an **optional keyfile** (any file) as a second key factor.
...
| **TPM 2.0 binding** | ⚠️ TPM *detection* (TBS) + machine-bound **DPAPI** sealing — not real TPM PCR sealing | ❌ | ✅ (true TPM) | ❌ |
| **Device binding** (MachineGuid) | ✅ optional | ❌ | ✅ | ❌ |
...
| **Source available / auditable** | ✅ source-available (eval license) | ✅ open source | ❌ closed | ✅ open source |
...
| 🔒 **Multi-Factor Binding** | Containers: password + 2FA secret + optional TPM 2.0 + optional Device-ID. Single-file encryption additionally supports an **optional keyfile** (any file type) as a second key factor, selectable in the GUI. |
...
- **Containers** (create / mount / change-password / integrity) bind **password + 2FA
  secret** (+ optional TPM / device binding). They do **not** offer a keyfile field — a
  long, strong passphrase plus the mandatory 2FA factor is the intended protection.
...
```
Password + [Keyfile — single-file encryption only] + 2FA Secret + TPM Secret + Device-ID
                          │
...
> are in **[LICENSE](LICENSE)** (free for personal evaluation).

Ship a single **`DGKN-Setup.exe`** — no separate DLLs to copy. Built with Inno Setup
from `installer/dgkn_setup.iss`; it bundles `dgkn_gui.exe` plus all runtime DLLs and Qt
plugins, creates Start-menu / optional Desktop shortcuts, and registers an uninstaller.
...
& "C:\Program Files\Inno Setup 7\ISCC.exe" "/O$([Environment]::GetFolderPath('Desktop'))" installer\dgkn_setup.iss
# -> DGKN-Setup.exe on the Desktop
```

**To install:** double-click `DGKN-Setup.exe` and follow the wizard. The app installs to
`%ProgramFiles%\DGKN@Labs\Crypto Suite` (or per-user without admin) and adds a Start-menu entry.
...
- The **WinFsp** driver installed (<https://winfsp.dev>) — it is a kernel-mode filesystem
  driver and is *not* bundled in the installer (drivers can't be shipped this way).
...
├── installer/        # Inno Setup script + dist/SFX packaging (PowerShell)
...
The dashboard shows the crypto config (cipher · KDF · 2FA · TPM · mounts) **plus live system
metrics** — CPU load, CPU temperature, RAM usage, and system-disk free/used — refreshed every
2 s with green/amber/red thresholds.
...
> ⚠️ With TPM binding: migration or hardware changes without a proper backup/recovery concept can lock you out permanently.
...
DGKN Crypto Suite is released under an **Evaluation License** — see **[LICENSE](LICENSE)**.
In short: you're free to **download, build, run, and test** it for personal,
non-commercial evaluation and to send feedback. Redistribution, modification beyond your
own testing, and commercial use require prior written permission. All rights reserved.

### Third-party open-source licenses

The DGKN Evaluation License covers **only DGKN's own code**. The Software is built on
third-party open-source libraries, each under its own license — these are unaffected by
the Evaluation License and you keep all rights those licenses grant you. The full
