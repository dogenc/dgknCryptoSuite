...
namespace dgkn::core {
    ...
    std::string ContainerManager::create_mount_dir([[maybe_unused]] const std::string& mode) {
        // SICHERHEIT: Der Volume-Modus ("normal"/"hidden") darf NICHT in den
        // Verzeichnisnamen. Frueher hiess der Ordner z. B. "DGKN7_hidden_<ts>" und blieb
        // (bei echtem Laufwerks-Mount leer) in %TEMP% zurueck — der Name allein bewies dann
        // forensisch, dass ein verstecktes Volume geoeffnet wurde, und hebelte die plausible
        // deniability aus. Stattdessen ein zufaelliges, nicht zuordenbares Suffix: weder der
        // Modus noch eine Reihenfolge ("welcher war hidden?") lassen sich ableiten.
        unsigned char rnd[8];
        randombytes_buf(rnd, sizeof(rnd));
        static const char* hexd = "0123456789abcdef";
        std::string token;
        for (unsigned char b : rnd) { token += hexd[b >> 4]; token += hexd[b & 0x0F]; }
        auto dir = fs::path(mount_root_dir()) /
            (std::string(config::MOUNT_DIR_PREFIX) + token);
        std::error_code ec; fs::create_directories(dir, ec);
        return dir.string();
    }
    ...
    OpResult ContainerManager::mount_volume(
        ...
        try {
            ...
            // SICHERHEIT: Die Mount-ID darf den Modus ("normal"/"hidden") NICHT enthalten.
            // Sie wird als r.message nach aussen gereicht (GUI/Logs); ein "hidden_..."-Praefix
            // wuerde die Existenz eines versteckten Volumes verraten. Stattdessen eine
            // zufaellige, opake ID — der Modus wird intern weiter in info->mode gefuehrt.
            unsigned char mid_rnd[12];
            randombytes_buf(mid_rnd, sizeof(mid_rnd));
            static const char* mid_hex = "0123456789abcdef";
            std::string mid;
            for (unsigned char b : mid_rnd) { mid += mid_hex[b >> 4]; mid += mid_hex[b & 0x0F]; }
