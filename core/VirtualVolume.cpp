...
namespace dgkn::core {
...
#if defined(_WIN32)
    // Diagnose-Log fuer die Mount-Fehlersuche: %TEMP%\dgkn_mount.log, sofort geflusht.
    ...
    static bool drive_root_visible(const std::string& mount_point) {
        ...
        // WICHTIG: Solange WinFsp den Buchstaben reserviert, aber das Volume noch nicht
        // bereit ist, kann ein Zugriff auf "Z:\" den modalen Windows-Hard-Error-Dialog
        // ("Z:\ ist nicht verfuegbar") ausloesen. SEM_FAILCRITICALERRORS unterdrueckt
        // diesen Popup waehrend der Pruefung; wir wollen nur das stille Ergebnis.
    ...
    bool VirtualVolume::mount(const std::string& mount_point) {
        ...
        loop_thread_ = std::thread([this, mount_point]() {
            ...
            std::string volprefix = "-ovolname=" + label_;
            ...
            mlog("thread: fuse_main_real START (argv1='%s', volprefix='%s')",
                 argv.size() > 1 ? argv[1] : "?", volprefix.c_str());
        ...
        mlog("mount() ENTER mp='%s' label='%s'", mount_point.c_str(), label_.c_str());
